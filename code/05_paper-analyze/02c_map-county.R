...
# Parquet collections to use
cvr_parquet_PATH = "~/Dropbox/CVR_parquet/release"
...
cvr_county <- arrow::open_dataset(cvr_parquet_PATH) |> 
...
all_county <- arrow::open_dataset(medsl_precinct_parquet_PATH,
                                  unify_schemas = FALSE,
  ...
  summarize(prec_votes = sum(votes),
            modes = list(mode),
            .by = c(state_po,
                    county_name, 
                    county_fips)) |>
  mutate(modes = map_chr(modes, \(x) str_flatten(sort(unique(x)), 
                                                 collapse = ";")),
         county_fips = str_pad(county_fips, width = 5, side = "left",
                               pad = "0")) |>
  left_join(cvr_county,
            by = c("state_po", "county_name")) |>
  arrange(state_po, county_name) |> 
  select(state, state_po, county_name, modes,
...
gg_county <- county_shp |>
  ...
  mutate(covered = case_when(!is.na(cvr_votes) ~ "c",
                             STATE_NAME %in% c("Delaware", 
                                               "Rhode Island") ~ "c",
  ...
  geom_sf(data = state_shp |> 
          ...
          linewidth = 0.6,
          fill = NA) +
  scale_fill_manual(values = c("a" = "#F0F0F0", 
                               "b" = "lightgrey", 
                               "c" = "black"), 
                    guide="none") +
  ggthemes::theme_map() 
...
state_map <- function(the_state) {
  county_shp |>
    ...
    mutate(covered = case_when(!is.na(cvr_votes) ~ TRUE,
                               STATE_NAME %in% c("Rhode Island",
                                                 "Delaware") ~ TRUE,
                               TRUE ~ FALSE),
    ...
    geom_sf(aes(fill = covered),
            color = "white") +
    scale_fill_manual(values = c("FALSE" = "lightgrey",  
                                 "TRUE" = "black"), 
                      guide="none") +
    ggthemes::theme_map() +
...
gg_county_grid <- cowplot::plot_grid(plotlist = mps, 
