...
dat_cand <- read_excel("combined/compare.xlsx", sheet = "by-cand-coalesced") |> 
  # for now, filter to candidates 
  filter(!(party_detailed %in% c("UNDERVOTE","WRITEIN"))) |> 
  filter(party_detailed %in% c("DEMOCRAT", "REPUBLICAN"))

dat_county <- read_excel("combined/compare.xlsx", sheet = 2)
...
gg_pop <- cand_dist_level |>
  ...
  labs(x = "Total Votes Cast", y = "% Difference", color = NULL)
...
gg_hist <-
  ...
  scale_x_continuous(labels = \(x) scales::percent(x, accuracy = 0.1, suffix = ""), 
                     breaks = c(-0.05, -0.025, 0, 0.01),
                     expand = expansion(add = c(0, 0.0001))) +
  labs(fill = NULL, x = "% Difference", y = "Percent of sample", 
       caption = "x-axis limited to [-0.05, +0.01]") +
...
ggsave("paper/figs/error_plots.png", w = 10, h = 3)

# 
# (cand_dist_level |>  filter(perc_diff == 0) |> nrow()) /
#   nrow(cand_dist_level |> filter(!is.na(perc_diff)))
# write_lines()

dat_county |> 
  filter(color2_c == "0 difference") |> 
  nrow() |> 
  write_lines("paper/numbers/counties_0-disc.tex")

dat_county |> 
  filter(color2_c == "any < 1% mismatch") |> 
  nrow() |> 
  write_lines("paper/numbers/counties_1perc-disc.tex")
