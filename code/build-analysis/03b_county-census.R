...
vars <- c("P1_001N", # total population
...
cen <- get_decennial(year = 2020,
  ...
  dplyr::rename(tot = P1_001N,
         ...
         his_race = P5_010N) |>
  mutate(prop_urb = tot_urb/tot_urb_rur * 100, #calculate proportion vars
         ...
         all18 = rowSums(across(c(P12_003N,
                                   P12_004N,
                                   P12_005N,
                                   P12_006N,
                                  P12_027N, P12_028N,
                                  P12_029N, P12_030N)),
                          na.rm = TRUE),
         all65 = rowSums(across(c(P12_020N, P12_021N,
                                   P12_022N, P12_023N,
                                   P12_024N, P12_025N,
                                  P12_044N, P12_045N,
                                  P12_046N, P12_047N,
         ...
         county = str_trim(str_extract(NAME,
                                       ".*(?= County)")),  # extract everything before "County"
         state = str_trim(str_extract(NAME,
                                      "(?<=, ).*")), # extract everything after common and space
         county = toupper(county)) |>
  select(-starts_with("P1"))

sum_vars <- c("prop_urb", "prop_white", # designate our final summary variables
...
summary_subset <- bind_rows(
  lapply(sum_vars,
         function(var) calc_summary_stats(cvr_cen,
                                          var))
)
...
sum_stats <- data.frame(
  ...
  Variable = rep(c("Percent Urban", "Percent White",
                   ...
                   "Percent Homeowning"), 3),
  `All U.S. Counties` = c(summary_all$Mean,
                          summary_all$Median,
                          summary_all$SD),
  `Our Data` = c(summary_subset$Mean,
